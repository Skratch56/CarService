...
<link rel="icon" href="black.jpg">
    ...
    <body>
        ...
        <div class="container">
             <?php
        if (isset($_GET['searchEVMSG']) == 'Not Found') {
            echo '<div class="alert alert-danger alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Not Found.</div>';
        }
        ?>
            <div class="panel panel-default">
                <div class="panel-heading">Enter Evaluation Details</div>
                        ...
                        <div class="row" id="marg-top-20">
                            <div class="col-md-6">
                                <div class="row">
                                    ...
                                    <div class="col-md-3">
                                        <?php
                                        echo '<input type="text" class="form-control" name="booking" id="booking" required="true" placeholder="Enter Evaluation" />'
                                        ?>
                                    </div>
            ...
            if (isset($_POST['btnsavebooking'])) {
                ...
                $query2 = "Select * from evaluation where evaluation_no='{$Booking}'";

                $result2 = mysqli_query($conn, $query2);
                if (mysqli_fetch_array($result2) > 0) {
                    $query3 = "Select * from service where evaluation_no='{$Booking}'";
                    $result3 = mysqli_query($conn, $query3);
                    if (mysqli_fetch_array($result3) > 0) {
                        echo "<script>
					alert('Evaluation services Have already been captured');
					</script>";
                        echo" <script language='JavaScript' type='text/javascript' >
					 window.location.href ='servicedetails.php?evid={$Booking}'
					</script>";
                    } else {
                       echo" <script language='JavaScript' type='text/javascript' >
					 window.location.href ='service.php?evid={$Booking}'
					</script>";
                    }
                    ...
                    header("Location: searchEvaluation.php?searchEVMSG='Not Found'");;
            }}
                ?>
